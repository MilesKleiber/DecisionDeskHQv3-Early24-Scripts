        ...
        if party_data['name'] == 'Republican':
            for state_data in party_data['states']:
                if state_data['state_name'] == setstate:
                    deleg_republican_data = state_data['candidates']
                    delegate_vote_total = state_data['total']
                    break
    deleg_republican_ids = list(deleg_republican_data.keys())

    for candidate in d_response_data['candidates']:
        candidate_id = str(candidate['cand_id'])
        if candidate_id in deleg_republican_ids:
            ...
            with open(state_deleg_dir + file_name, 'w') as file:
                file.write(str(deleg_republican_data[candidate_id]))
            if delegate_vote_total <= 0.0:
                cand_percent = 0.0
            else:
                cand_percent = float(deleg_republican_data[candidate_id] / delegate_vote_total) * 100
